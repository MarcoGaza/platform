...
/** Judge whether to stop mouse move event processing */
...
function handleScroll(index: number) {
  ...
  const scrollTop = curRef.offsetTop + 128; // 128 Twohistory-item（56px+56px=112px）Height plus up and downmargin（8px+8px=16px）
...
<template>
  <div ref="historyRef" class="history">
    <template v-if="historyList.length">
      <div :style="titleStyle">Search history</div>
      <div
    ...
    <template v-if="collectList.length">
      <div :style="titleStyle">
        {{
          `Collection${collectList.length > 1 ? "（Drag and drop sort）" : ""}`
        }}
      </div>
